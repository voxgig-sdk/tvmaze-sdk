...
[MCP](https://modelcontextprotocol.io) server exposing the Tvmaze SDK as
two agent tools — `tvmaze_list` and `tvmaze_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tvmaze-mcp)
make build

# 2. Provide credentials via the environment
export TVMAZE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user tvmaze \
  -- /absolute/path/to/tvmaze-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./tvmaze-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// tvmaze_list: first page of records
{ "entity": "aka" }
{ "entity": "aka", "query": { } }

// tvmaze_load: one record by id
{ "entity": "alternate_list", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tvmaze-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user tvmaze \
     -- "$PWD"/dist/*/tvmaze-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `tvmaze_list` and `tvmaze_load` tools now appear
   in new sessions. Ask the agent to *"list aka using tvmaze"*
   and it calls `tvmaze_list` with `{"entity":"aka"}`.
...
```sh
export TVMAZE_APIKEY=sk_live_xxx            # API key
export TVMAZE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `tvmaze_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `tvmaze_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TVMAZE_APIKEY` | API key sent with every request. |
| `TVMAZE_BASE` | Optional override of the API base URL. |
...
```sh
./tvmaze-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"tvmaze_load","arguments":{"entity":"alternate_list","query":{"id":1}}}}'
```
