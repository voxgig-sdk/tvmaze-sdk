...
from tvmaze_sdk.tvmaze_types import (
    AlternateList,
...
class AlternateListEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AlternateListEntity(self._client, opts)
    ...
    def data_get(self) -> AlternateList:
    ...
    def match_get(self) -> AlternateList:
    ...
    def load(self, reqmatch=None, ctrl=None) -> AlternateList:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.AlternateList().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[AlternateList]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.AlternateList().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
