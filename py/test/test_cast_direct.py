...
class TestCastDirect:
    ...
    def test_should_direct_list_cast(self):
        setup = _cast_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-cast", "live" if setup["live"] else "unit")
        if _skip:
...
def _cast_direct_setup(mockres):
