...
class TestSeasonDirect:

    def test_should_direct_list_season(self):
        setup = _season_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-season", "live" if setup["live"] else "unit")
        if _skip:
...
def _season_direct_setup(mockres):
