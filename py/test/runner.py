...
class TvmazeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TvmazeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TvmazeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TvmazeTestRunner.getenv("TVMAZE_TEST_LIVE")
        override = TvmazeTestRunner.getenv("TVMAZE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TvmazeTestRunner.getenv(key)
        ...
        explain = TvmazeTestRunner.getenv("TVMAZE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TVMAZE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TvmazeTestRunner._test_control is not None:
            return TvmazeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TvmazeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TvmazeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TvmazeTestRunner.load_test_control()
...
def load_env_local():
    TvmazeTestRunner.load_env_local()


def env_override(m):
    return TvmazeTestRunner.env_override(m)


def entity_data(v):
    return TvmazeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TvmazeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TvmazeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TvmazeTestRunner.load_test_control()


def live_client_options():
    return TvmazeTestRunner.live_client_options()


def live_delay_ms():
    return TvmazeTestRunner.live_delay_ms()
