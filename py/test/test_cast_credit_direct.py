...
class TestCastCreditDirect:

    def test_should_direct_list_cast_credit(self):
        setup = _cast_credit_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-cast_credit", "live" if setup["live"] else "unit")
        if _skip:
...
def _cast_credit_direct_setup(mockres):
