...
func TestShowEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Show(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Show(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		showRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.show")))
		...
		showRef01DataDt0LoadResult := core.ToMapAny(entityData(showRef01DataDt0Loaded))
