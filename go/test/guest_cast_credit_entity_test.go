...
func TestGuestCastCreditEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GuestCastCredit(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GuestCastCredit(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		guestCastCreditRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.guest_cast_credit")))
