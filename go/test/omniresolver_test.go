...
import (
	...
	omni "github.com/voxgig-sdk/tvmaze-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TvmazeError); is {
				out := map[string]any{
					"name":    "TvmazeError",
...
func omniCtx(arg any, client *sdk.TvmazeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TvmazeSDK); is {
			client = live
