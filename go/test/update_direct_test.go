...
func TestUpdateDirect(t *testing.T) {
	t.Run("direct-load-update", func(t *testing.T) {
		setup := updateDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-update", _mode); _shouldSkip {
			if _reason == "" {
...
func updateDirectSetup(mockres any) *updateDirectSetupResult {
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["TVMAZE_TEST_UPDATE_ENTID"]; ok {
