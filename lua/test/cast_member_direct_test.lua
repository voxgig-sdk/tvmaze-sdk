...
describe("CastMemberDirect", function()
  it("should direct-list-cast_member", function()
    local setup = cast_member_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-cast_member", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"episode01"}) do
...
function cast_member_direct_setup(mockres)
