...
The Lua SDK for the Tvmaze API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Aka(nil):list(nil, nil)
...
local result, err = client:Tvmaze(nil):load(
  { id = "test01" }, nil
)
