...
local CrewMemberEntity = {}
CrewMemberEntity.__index = CrewMemberEntity


function CrewMemberEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CrewMemberEntity)
  self._name = "crew_member"
  self._client = client
...
function CrewMemberEntity:get_name()
  return self._name
end


function CrewMemberEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CrewMemberEntity.new(self._client, opts)
end
...
function CrewMemberEntity:mark_deleted()
  self._deleted = true
end


function CrewMemberEntity:deleted()
  return true == self._deleted
end


function CrewMemberEntity:data_set(args)
  if args ~= nil then
...
function CrewMemberEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CrewMemberEntity:match_set(args)
  if args ~= nil then
...
function CrewMemberEntity:match_get()
...
function CrewMemberEntity:stream(action, args, callopts)
...
function CrewMemberEntity:_run_op(ctx, post_done)
  local utility = self._utility
