...
local ShowEntity = {}
ShowEntity.__index = ShowEntity


function ShowEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShowEntity)
  self._name = "show"
  self._client = client
...
function ShowEntity:get_name()
  return self._name
end


function ShowEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShowEntity.new(self._client, opts)
end
...
function ShowEntity:mark_deleted()
  self._deleted = true
end


function ShowEntity:deleted()
  return true == self._deleted
end


function ShowEntity:data_set(args)
  if args ~= nil then
...
function ShowEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShowEntity:match_set(args)
  if args ~= nil then
...
function ShowEntity:stream(action, args, callopts)
...
---@param reqmatch ShowLoadMatch
---@param ctrl? table
---@return Show
---@return string? err
function ShowEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ShowListMatch
---@param ctrl? table
---@return Show[]
---@return string? err
function ShowEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ShowEntity:_run_op(ctx, post_done)
  local utility = self._utility
