...
local AkaEntity = {}
AkaEntity.__index = AkaEntity


function AkaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AkaEntity)
  self._name = "aka"
  self._client = client
...
function AkaEntity:get_name()
  return self._name
end


function AkaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AkaEntity.new(self._client, opts)
end
...
function AkaEntity:mark_deleted()
  self._deleted = true
end


function AkaEntity:deleted()
  return true == self._deleted
end


function AkaEntity:data_set(args)
  if args ~= nil then
...
function AkaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AkaEntity:match_set(args)
  if args ~= nil then
...
function AkaEntity:stream(action, args, callopts)
...
function AkaEntity:_run_op(ctx, post_done)
  local utility = self._utility
