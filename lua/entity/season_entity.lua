...
local SeasonEntity = {}
SeasonEntity.__index = SeasonEntity


function SeasonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SeasonEntity)
  self._name = "season"
  self._client = client
...
function SeasonEntity:get_name()
  return self._name
end


function SeasonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SeasonEntity.new(self._client, opts)
end
...
function SeasonEntity:mark_deleted()
  self._deleted = true
end


function SeasonEntity:deleted()
  return true == self._deleted
end


function SeasonEntity:data_set(args)
  if args ~= nil then
...
function SeasonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SeasonEntity:match_set(args)
  if args ~= nil then
...
function SeasonEntity:match_get()
...
function SeasonEntity:stream(action, args, callopts)
...
---@param reqmatch SeasonListMatch
---@param ctrl? table
---@return Season[]
---@return string? err
function SeasonEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SeasonEntity:_run_op(ctx, post_done)
  local utility = self._utility
