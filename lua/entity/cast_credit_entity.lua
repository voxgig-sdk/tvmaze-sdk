...
local CastCreditEntity = {}
CastCreditEntity.__index = CastCreditEntity


function CastCreditEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CastCreditEntity)
  self._name = "cast_credit"
  self._client = client
...
function CastCreditEntity:get_name()
  return self._name
end


function CastCreditEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CastCreditEntity.new(self._client, opts)
end
...
function CastCreditEntity:mark_deleted()
  self._deleted = true
end


function CastCreditEntity:deleted()
  return true == self._deleted
end


function CastCreditEntity:data_set(args)
  if args ~= nil then
...
function CastCreditEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CastCreditEntity:match_set(args)
  if args ~= nil then
...
function CastCreditEntity:match_get()
...
function CastCreditEntity:stream(action, args, callopts)
...
---@param reqmatch CastCreditListMatch
---@param ctrl? table
---@return CastCredit[]
---@return string? err
function CastCreditEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CastCreditEntity:_run_op(ctx, post_done)
  local utility = self._utility
