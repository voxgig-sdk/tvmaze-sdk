...
local CastEntity = {}
CastEntity.__index = CastEntity


function CastEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CastEntity)
  self._name = "cast"
  self._client = client
...
function CastEntity:get_name()
  return self._name
end


function CastEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CastEntity.new(self._client, opts)
end
...
function CastEntity:mark_deleted()
  self._deleted = true
end


function CastEntity:deleted()
  return true == self._deleted
end


function CastEntity:data_set(args)
  if args ~= nil then
...
function CastEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CastEntity:match_set(args)
  if args ~= nil then
...
function CastEntity:stream(action, args, callopts)
...
function CastEntity:_run_op(ctx, post_done)
  local utility = self._utility
