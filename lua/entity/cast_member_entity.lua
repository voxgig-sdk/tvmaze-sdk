...
local CastMemberEntity = {}
CastMemberEntity.__index = CastMemberEntity


function CastMemberEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CastMemberEntity)
  self._name = "cast_member"
  self._client = client
...
function CastMemberEntity:get_name()
  return self._name
end


function CastMemberEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CastMemberEntity.new(self._client, opts)
end
...
function CastMemberEntity:mark_deleted()
  self._deleted = true
end


function CastMemberEntity:deleted()
  return true == self._deleted
end


function CastMemberEntity:data_set(args)
  if args ~= nil then
...
function CastMemberEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CastMemberEntity:match_set(args)
  if args ~= nil then
...
function CastMemberEntity:match_get()
...
function CastMemberEntity:stream(action, args, callopts)
...
---@param reqmatch CastMemberListMatch
---@param ctrl? table
---@return CastMember[]
---@return string? err
function CastMemberEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CastMemberEntity:_run_op(ctx, post_done)
  local utility = self._utility
