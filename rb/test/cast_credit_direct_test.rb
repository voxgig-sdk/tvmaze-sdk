...
class CastCreditDirectTest < Minitest::Test
  def test_direct_list_cast_credit
    setup = cast_credit_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-cast_credit", setup[:live] ? "live" : "unit")
    if _should_skip
...
def cast_credit_direct_setup(mockres)
