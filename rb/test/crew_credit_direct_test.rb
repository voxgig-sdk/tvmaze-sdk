...
class CrewCreditDirectTest < Minitest::Test
  def test_direct_list_crew_credit
    setup = crew_credit_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-crew_credit", setup[:live] ? "live" : "unit")
    if _should_skip
...
def crew_credit_direct_setup(mockres)
