...
class CrewEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TVMAZE_TEST_CREW_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def crew_basic_setup(extra)
  ...
  idmap = Vs.transform(
    ["crew01", "crew02", "crew03", "show01", "show02", "show03"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
  ...
  entid_env_raw = ENV["TVMAZE_TEST_CREW_ENTID"]
