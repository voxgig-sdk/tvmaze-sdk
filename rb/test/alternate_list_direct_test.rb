...
class AlternateListDirectTest < Minitest::Test
  def test_direct_list_alternate_list
    setup = alternate_list_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-alternate_list", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["show01"].each do |_live_key|
  ...
  def test_direct_load_alternate_list
    setup = alternate_list_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-alternate_list", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def alternate_list_direct_setup(mockres)
