...
class AkaDirectTest < Minitest::Test
  def test_direct_list_aka
    setup = aka_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-aka", setup[:live] ? "live" : "unit")
    if _should_skip
