...
class AlternateListEntity
  ...
  def make
    opts = @_entopts.dup
    AlternateListEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.AlternateList.load works with no args).
  ...
  #
  # @param reqmatch [AlternateListListMatch, Hash, nil] match filter (any subset of
  #   AlternateList fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
