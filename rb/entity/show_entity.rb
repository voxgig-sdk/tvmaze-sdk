...
class ShowEntity
  ...
  def make
    opts = @_entopts.dup
    ShowEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TvmazeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Show, Hash] the current Show data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TvmazeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Show fields)
  def match_get
  ...
  # Load a single Show.
  #
  # @param reqmatch [ShowLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Show.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Show, Hash] the loaded Show; raises TvmazeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TvmazeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ShowListMatch, Hash, nil] match filter (any subset of
  #   Show fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Show>, Array] the matching Show items; raises TvmazeError on failure
