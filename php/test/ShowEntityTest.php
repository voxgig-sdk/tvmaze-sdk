...
class ShowEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TvmazeSDK::test(null, null);
        $ent = $testsdk->Show(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Show(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Show(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = show_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "show." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TVMAZE_TEST_SHOW_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $show_ref01_data = null;
        if (count($show_ref01_data_raw) > 0) {
            $show_ref01_data = Helpers::to_map($show_ref01_data_raw[0][1]);
        }

        // LIST
        $show_ref01_ent = $client->Show(null);
        $show_ref01_match = [];

        $show_ref01_list_result = $show_ref01_ent->list($show_ref01_match, null);
        $this->assertIsArray($show_ref01_list_result);
...
function show_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/show/ShowTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["show01", "show02", "show03", "alternatelist01", "alternatelist02", "alternatelist03"] as $k) {
    ...
    $entid_env_raw = getenv("TVMAZE_TEST_SHOW_ENTID");
