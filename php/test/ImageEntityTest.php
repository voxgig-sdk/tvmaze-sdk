...
class ImageEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TVMAZE_TEST_IMAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function image_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["image01", "image02", "image03", "show01", "show02", "show03"] as $k) {
    ...
    $entid_env_raw = getenv("TVMAZE_TEST_IMAGE_ENTID");
