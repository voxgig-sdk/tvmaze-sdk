...
// Tvmaze SDK AlternateList entity
...
class AlternateListEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AlternateListEntity($this->_client, $opts);
    }

    /**
     * @param AlternateList|array $args AlternateList data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TvmazeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return AlternateList|array The current AlternateList data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of AlternateList fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TvmazeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of AlternateList fields).
     */
     ...
     *
     * @param AlternateListLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AlternateListLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return AlternateList|array The loaded AlternateList as an assoc-array at the
     *   SDK boundary; throws TvmazeError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TvmazeHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param AlternateListListMatch|array|null $reqmatch Match filter (any subset
     *   of AlternateList fields) as an assoc-array; AlternateListListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return AlternateList[]|array A list of AlternateList items as assoc-arrays at
     *   the SDK boundary; throws TvmazeError on failure (item-5 convention).
     */
