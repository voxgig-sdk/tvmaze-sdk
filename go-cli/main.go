// Command Tvmaze-cli is a CLI + REPL for the Tvmaze SDK,
...
const entitiesHelp = "aka alternate_list cast cast_credit cast_member crew crew_credit crew_member episode guest_cast_credit image person schedule scheduled_episode search season show update"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TVMAZE_APIKEY (api key), TVMAZE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TVMAZE_APIKEY carries the API key and
	// TVMAZE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TVMAZE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TVMAZE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTvmazeSDK(opts)
