...
boru-driven command-line client **and** interactive REPL for the Tvmaze
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tvmaze-cli)
make build
...
./tvmaze-cli list aka
./tvmaze-cli list alternate_list

# 5. Override the API base URL for a single call
TVMAZE_BASE=https://api.example.com ./tvmaze-cli list aka
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tvmaze-cli
   ```
...
Run with no arguments to open a REPL (prompt `tvmaze>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `aka`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/tvmaze-cli`. |
...
(`./tvmaze-cli <expr>`) and interactively (the REPL), and expressions compose the
