...
{
    private static function utility(): TvmazeUtility
    {
        return new TvmazeUtility();
    }
    ...
    private static function ctx(array $over = []): TvmazeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TvmazeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TvmazeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TvmazeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TvmazeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TvmazeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TvmazeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TvmazeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TvmazeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TvmazeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TvmazeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TvmazeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TvmazeResponse([]),
            'result' => new TvmazeResult([]),
        ]);
        [, $err] = TvmazeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TvmazeSpec(['step' => 's']),
            'response' => new TvmazeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TvmazeResult([]),
        ]);
        [, $err] = TvmazeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TvmazeSpec(['step' => 's']),
            'response' => new TvmazeResponse(self::resp(200, ['v' => 1])),
            'result' => new TvmazeResult([]),
        ]);
        [, $err] = TvmazeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TvmazeSpec(['step' => 's']),
            'response' => new TvmazeResponse(self::resp(200, ['v' => 2])),
            'result' => new TvmazeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TvmazeMakeResponse::call($ctx);
    ...
    {
        $preset = new TvmazeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TvmazeSpec([]),
            'response' => new TvmazeResponse([]),
            'result' => new TvmazeResult([]),
        ...
        [$got, $err] = TvmazeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TvmazeResult([])]);
        [, $err] = TvmazeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TvmazeSpec([]), 'result' => null]);
        [, $err] = TvmazeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TvmazeSpec(['step' => 's']),
            'result' => new TvmazeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TvmazeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TvmazeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TvmazeSpec(['step' => 's']),
            'result' => new TvmazeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TvmazeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TvmazeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TvmazeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TvmazeSpec([]),
            'result' => new TvmazeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TvmazeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TvmazeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TvmazeError('boom', 'boom');
        $utility->fetcher = function (TvmazeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TvmazeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TvmazeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TvmazeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TvmazeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TvmazeResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TvmazeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TvmazeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TvmazeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TvmazeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TvmazeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TvmazeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TvmazeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TvmazeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TvmazeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TvmazeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TvmazeResult(['ok' => false])]);
        $this->expectException(TvmazeError::class);
        TvmazeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TvmazeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TvmazeMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TvmazeBaseFeature();
        $b = new TvmazeBaseFeature();
        TvmazeFeatureAdd::call($ctx, $a);
        TvmazeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TvmazeBaseFeature
    {
        $f = new TvmazeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TvmazeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TvmazeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TvmazeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TvmazePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TvmazeResponse(['headers' => null]),
            'result' => new TvmazeResult([]),
        ]);
        TvmazeResultHeaders::call($ctx);
