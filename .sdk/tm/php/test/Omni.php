...
//    only - a typed TvmazeContext there would make every ctx
...
final class TvmazeOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TvmazeContext($clean, null);
    ...
    public static function err_from_map($m): ?TvmazeError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TvmazeError(is_string($code) ? $code : '', $msg);
    }
