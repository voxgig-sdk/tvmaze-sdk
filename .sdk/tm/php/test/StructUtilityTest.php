...
// Tvmaze SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TvmazeOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TvmazeOmni::structfix($now, Struct::undef()))
                        !== json_encode(TvmazeOmni::structfix($val, Struct::undef()))) {
