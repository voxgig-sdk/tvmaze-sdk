...
class FtCtrl extends TvmazeControl
{
...
class FtClient
{
    ...
    private TvmazeUtility $utility;

    public function __construct(TvmazeUtility $utility, array $options)
    {
    ...
    public function get_utility(): TvmazeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TvmazeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TvmazeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TvmazeConfig::make_feature($name);
    ...
    private static function buildUrl(TvmazeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TvmazeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TvmazeResponse($fetched);
        }
    ...
    private function fail(TvmazeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TvmazeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
