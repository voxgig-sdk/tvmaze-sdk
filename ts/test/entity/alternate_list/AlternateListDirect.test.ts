...
describe('AlternateListDirect', async () => {
  ...
  test('direct-load-alternate_list', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-alternate_list', setup.live)) return
  ...
  test('direct-list-alternate_list', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-alternate_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["show01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TVMAZE_TEST_ALTERNATE_LIST_ENTID']
